...
export default function AccountPage({ params }) {
    ...
    // ... (Keep your existing useEffect for session checking)
    useEffect(() => {
    ...
    // ✨ UPDATED LOGIN / REGISTER HANDLER
    const handleAuthSubmit = async (e) => {
        ...
        try {
            ...
            if (data.user.role === 'Customer') {
                // Customer -> Redirect to the home (product) page
                router.push('/');
            }
            // Vendor/Admin -> Stay on this page to show the "Open Admin Panel" button
    ...
    // ... (Keep handleLogout and handlePasswordReset exactly as they were)
    const handleLogout = () => {
    ...
    // ==========================================
    if (!user) {
        // ... (Keep the login/register form JSX exactly as it was)
        return (
    ...
    if (userRole === 'VendorAdmin' || userRole === 'VendorStaff') {
        return (
            <div className="container mx-auto px-4 py-16 max-w-3xl">
                {/* ... (Keep the Management Portal JSX exactly as it was) */}
    ...
    // CUSTOMER VIEW (Shows when they visit /account manually after login)
    return (
        <div className="container mx-auto px-4 py-12 max-w-6xl">
            {/* ... (Keep the Customer Dashboard JSX exactly as it was) */}
            ...
            <div className="flex flex-col md:flex-row gap-10">
                ...
                <div className="flex-1 bg-white border border-gray-100 rounded-3xl p-6 sm:p-10 shadow-sm">
                    {activeTab === 'orders' && (/* orders JSX */ <div>...</div>)}
                    {activeTab === 'profile' && (/* profile JSX */ <div>...</div>)}
                    {activeTab === 'security' && (/* security JSX */ <div>...</div>)}
                </div>
            </div>
        </div>
    );
