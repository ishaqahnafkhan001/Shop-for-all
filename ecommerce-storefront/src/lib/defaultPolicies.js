export const POLICY_TYPES = ["refund", "shipping", "privacy", "terms"];
...
export const DEFAULT_POLICY_TEMPLATES = {
    refund: `Thank you for shopping with {{STORE_NAME}}. We want our customers to have a smooth and reliable shopping experience.
...
For refund or return support, please contact us using the contact information provided on our store.`,
    shipping: `At {{STORE_NAME}}, we try to deliver orders safely and on time.
...
If a parcel is delayed, lost, or damaged during delivery, customers should contact us so we can coordinate with the courier service.`,
    privacy: `{{STORE_NAME}} respects customer privacy and is committed to protecting personal information.
...
By using this store, customers agree to the collection and use of information as described in this Privacy Policy.`,
    terms: `Welcome to {{STORE_NAME}}. By using this store and placing an order, customers agree to the following terms and conditions.
...
export const injectPolicyPlaceholders = (body = "", context = {}) => {
    const storeName = cleanValue(context.storeName || context.shopName, "this store");
    const contactEmail = cleanValue(context.contactEmail);
    const contactPhone = cleanValue(context.contactPhone);
...
export const getDefaultPolicyText = (type, context = {}) => {
    ...
    if (!template) return "";
    return injectPolicyPlaceholders(template, context);
};
...
export const getPolicyContent = (policies = {}, type, context = {}) => {
    const saved = String(policies?.[type] || "").trim();
    return saved || getDefaultPolicyText(type, context);
};
