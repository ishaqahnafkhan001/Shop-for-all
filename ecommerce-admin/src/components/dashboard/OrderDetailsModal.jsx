        ...
        >
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <h3 className="text-sm font-bold text-gray-900 mb-3 flex items-center">
                                <User size={16} className="mr-2 text-indigo-600" /> Customer
                            </h3>
                        ...
                        {/* Shipping Info */}
                        {/* FIX: was a broken <p> wrapping fragments. Now a proper card div */}
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <h3 className="text-sm font-bold text-gray-900 mb-3 flex items-center">
                                <MapPin size={16} className="mr-2 text-indigo-600" /> Shipping
                            </h3>
                            {order.shipping?.address ? (
                                <div className="text-sm text-gray-700 space-y-0.5">
                                    <p className="font-semibold">{order.shipping.address.fullName}</p>
                        ...
                        <div className="border border-gray-100 rounded-xl overflow-hidden">
                            <table className="min-w-full divide-y divide-gray-100">
                                ...
                                {order.items.map((item, index) => (
                                    <tr key={index}>
                                        {/* FIX: items are snapshots — use item.title, not item.product?.title */}
                    ...
                    {/* Cost Summary */}
                    {/* FIX: was using order.totalAmount and order.shippingCost (don't exist)
                             backend schema uses order.pricing.total and order.pricing.shipping */}
