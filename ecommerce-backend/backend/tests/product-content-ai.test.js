...
test('product content AI route is protected and mounted before product id route', () => {
    ...
    assert.match(adminRoutes, /INVALID_IMAGE_TYPE/);
    assert.match(adminRoutes, /IMAGE_TOO_LARGE/);
});
...
test('product content AI builds text-only and image-aware prompts without private pricing', async () => {
    ...
    const imagePart = await __test.buildImagePart({
        file: {
    ...
    assert.equal(imagePart.inlineData.data, Buffer.from('fake-image').toString('base64'));
});
...
test('product content AI normalizes shapes and length limits', () => {
    const cleaned = cleanProductContentSuggestion({
        ...
        sellingPoints: ['Traditional design for festive outfits', 'Traditional design for festive outfits', 'x'.repeat(140)],
        specifications: [
    ...
    assert.ok(cleaned.sellingPoints.length <= 6);
    assert.equal(cleaned.sellingPoints.length, 2);
    assert.ok(cleaned.sellingPoints.every(item => item.length <= 90));
...
test('product content AI malformed JSON path falls back instead of returning 500', () => {
    const controller = read('controllers/productController.js');
...
test('admin product AI preview flow maps suggestions without silent overwrite', () => {
    ...
    assert.match(addProduct, /getFirstImage=\{\(\) => imageFiles\[0\]/);
